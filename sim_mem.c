...
Instuction** instructions; // where the instructions are kept
...
int get_src2_imm(char *src2, int inst_num,int id) {
	instructions[id][inst_num].isSrc2Imm = 0; //assert
    strtok(src2, ",");
    strtok(NULL, ",");
    src2 = strtok(NULL, ",");
    if (strchr(src2, '$') == NULL) {
        strtok(src2, " ");
        instructions[id][inst_num].isSrc2Imm = 1;
    } else {
        strtok(src2, "$");
        src2 = strtok(NULL, "$");
        assert(instructions[id][inst_num].isSrc2Imm == 0);
    }
    src2 = strtok(src2, "\n");
...
void add_sub(char *line, int inst_num, int id) {
    char dst[50];
    memset(dst, '\0', sizeof(dst));
    strcpy(dst, line);
    instructions[id][inst_num].dst_index = get_dst(dst);
    char src1[50];
    memset(src1, '\0', sizeof(src1));
    strcpy(src1, line);
    instructions[id][inst_num].src1_index = get_src1(src1);
    char src2[50];
    memset(src2, '\0', sizeof(src2));
    strcpy(src2, line);
    instructions[id][inst_num].src2_index_imm = get_src2_imm(src2, inst_num,id);
}

void halt(char *line, int inst_num, int id) {
    char dst[50];
    memset(dst, '\0', sizeof(dst));
    strcpy(dst, line);
    instructions[id][inst_num].dst_index = get_dst(dst);
    instructions[id][inst_num].isSrc2Imm=0;
    instructions[id][inst_num].src1_index=0;
    instructions[id][inst_num].src2_index_imm=0;
}


void load_store(char *line, int inst_num, int id) {
    char dst[50];
    memset(dst, '\0', sizeof(dst));
    strcpy(dst, line);
    instructions[id][inst_num].dst_index = get_dst(dst);
...
void get_inst(char *line, int inst_num, int id) {
    char command[50];
    ...
    instructions[id][inst_num].opcode = opc;
    switch (opc) {
        case CMD_NOP: // NOP
            break;
        case CMD_ADDI:
        case CMD_SUBI:
            add_sub(line, inst_num, id);
            break;
        case CMD_ADD:
        case CMD_SUB:
            add_sub(line, inst_num, id);
        ...
        case CMD_LOAD:
        case CMD_STORE:
            load_store(line, inst_num, id);
            break;
        case CMD_HALT:
            halt(line, inst_num,id);
            break;
    }
}
...
void SIM_MemInstRead(int addr, Instuction *dst, int id) {
    ...
    dst->src2_index_imm = instructions[id][addr].src2_index_imm;
    dst->isSrc2Imm = instructions[id][addr].isSrc2Imm;
}

int Mem_latency(int *array) {
	array[0]=load_store_latency[0];
	array[1]=load_store_latency[1];
    return 0;
}
